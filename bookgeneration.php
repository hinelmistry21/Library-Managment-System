...
if(isset($_POST['ADD']))
{
  ...
  fclose($myfile);
  $tag=hexdec($tagr);
  //This will be used for checking whether the tag is red or not
  if($tag!=null){
    $bookavailable=$_POST['book_avai'];
    $bookname=$_POST['book_name'];
    $bookauthor=$_POST['author_name'];
    $bookedition=$_POST['book_edition'];
    $data = mysqli_query($conn,"INSERT IGNORE INTO book(Book_name,Book_author,Book_edition,Book_availability,TagID)
    VALUES('$bookname','$bookauthor','$bookedition','$bookavailable','$tag')");
      if($data)
      {
